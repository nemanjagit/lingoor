...
@Component({
  ...
  templateUrl: './personalized-feed.html',
  styleUrl: './personalized-feed.scss',
})
export class PersonalizedFeed implements OnInit {
  posts: Post[] = [];
  currentQuery = '';
  currentSort = 'newest';
  currentPage = 0;
  ...
  loadPosts(reset: boolean = false) {
    ...
    this.postsService
      .getPersonalizedFeed(this.currentQuery, this.currentSort, pageToLoad, this.pageSize)
      .subscribe({
        next: (res) => {
